...
export interface CompleteHocuspocusProviderConfiguration {
  ...
  /**
   * The size of the message chunks that are sent to the server.
   * if 0 or undefined, the message will be sent as a single chunk.
   */
...
export class HocuspocusProvider extends EventEmitter {
  ...
  send(message: ConstructableOutgoingMessage, args: any) {
    ...
    messageSender.send(this.configuration.websocketProvider, {
      messageChunkSize: this.configuration.messageChunkSize,
    });
  }

  onMessage(event: MessageEvent, dataOverride?: ArrayBufferLike) {
    const message = new IncomingMessage(dataOverride || event.data);

    const documentName = message.readVarString();

    message.writeVarString(documentName);

    this.emit('message', { event, message: new IncomingMessage(dataOverride || event.data) });
